...
export default function Adminhome() {
  ...
  useEffect(() => {
    const fetchLoans = async () => {
      try {
        const response = await axios.get('ingenious-expression-production.up.railway.app/allusers');
  ...
  const viewPdf = async (email, loanType) => {
    try {
      const response = await axios.get(
        `ingenious-expression-production.up.railway.app/api/pdf/view/${email}/${loanType}`,
        {
          responseType: 'blob',
        }
      );
  ...
  const handleStatusChange = async (email, loanType, status) => {
    try {
      ...
      const response = await axios.post("ingenious-expression-production.up.railway.app/updateStatus", params, {
  ...
  const handleLogout = () => {
    ...
    // Implement your logout logic, such as clearing tokens or redirecting to login
    window.location.href = "/"; // Example redirect
  };

  return (
    <Box sx={{ padding: 5, backgroundColor: '#006064', minHeight: '100vh', position: 'relative' }}>
      {/* Logout Button */}
      <Button
        variant="contained"
        color="secondary"
